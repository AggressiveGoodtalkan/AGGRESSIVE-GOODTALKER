...
module.exports = {
  ...
  run: async (bot, message, args) => {
    ...
    .get('694810450621366291').members
    .map(m => m).sort();
    ...
    const online = message.guild.members.cache.filter(
        m => m.user.presence.status === 'online' || m.user.presence.status === 'idle' || m.user.presence.status === 'dnd'
    );
    ...
    .addFields(
        { name: `**Users (Online/Total)**`, value: `${online.size}/${message.guild.memberCount - bots}`, inline: true },
        { name: ` **Created At:**`, value:`${createdAt.toLocaleTimeString('en-US')}\n${createdAt.toLocaleDateString('en-US', options)}`, inline: true },
        { name: `**Voice/Text Channels**`, value: `${voiceChannel}/${textChannel}`, inline: true },
        { name: `**Server Owner:**`, value:`${message.guild.owner.user.tag}`, inline: true},
        { name: `**Region**`, value: `${message.guild.region[0].toUpperCase() + message.guild.region.slice(1)}`, inline: true},
        { name: `**Role Count:**`, value:`${message.guild.roles.cache.size}`, inline: true },
        { name: `**Modmins:** `, value:`${adminList}`, inline: true },
